...
void btocl_AllocLhInfo(TREES* Trees) {
	...
	Trees->buffer_pmatrix  = clCreateBuffer(context, CL_MEM_READ_WRITE, 
		sizeof(double)*max_nnodes*NOS*NOS, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for pmatrix\n");
		exit(0);
	}	
	Trees->buffer_error  = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
		sizeof(int), NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for pmatrix\n");
		exit(0);
	...
	Trees->buffer_partialLh  = clCreateBuffer(context, CL_MEM_READ_WRITE, 
		sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS, NULL, &err);		
	if (err != 0) {
	...
	Trees->buffer_plhFactor  = clCreateBuffer(context, CL_MEM_READ_WRITE, 
		sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS, NULL, &err);	
	//Trees->buffer_plhFactor  = clCreateBuffer(context, CL_MEM_READ_WRITE, 
	//	sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS*Trees->max_nchildren, NULL, &err);	
	if (err != 0) {
			printf("Error allocating OpenCL buffer for children indexing\n");
			exit(0);
	}
	...
	Trees->previous_plh = (double*)malloc(sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS);
	Trees->temp_plh = (double*)malloc(sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS);
	Trees->plhFactor = (double*)malloc(sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS);
	//Trees->plhFactor = 
	//	(double*)malloc(sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS*Trees->max_nchildren);
	Trees->check_pmatrix = (double*)malloc(sizeof(double)*max_nnodes*NOS*NOS);
	
	
	Trees->debug_plhFactor = (double*)malloc(sizeof(double)*(Trees->NoOfSites)*max_nnodes*NOS);
...
void btocl_AllocInvInfo(cl_context context, INVINFO* InvInfo, int NOS, int MaxNodes) {
	...
	InvInfo->buffer_vec  = clCreateBuffer(context, CL_MEM_READ_ONLY, 
		sizeof(double)*NOS*NOS, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for EigenVector matrix\n");
		exit(0);
	}
	// cl_mem buffer_inv_vec  -  NOS*NOS  read-only
	InvInfo->buffer_inv_vec  = clCreateBuffer(context, CL_MEM_READ_ONLY, 
		sizeof(double)*NOS*NOS, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for the inverse of EigenVector matrix\n");
		exit(0);
	}
	// 	cl_mem buffer_val  -  NOS vector read-only
	InvInfo->buffer_val  = clCreateBuffer(context, CL_MEM_READ_ONLY, 
		sizeof(double)*NOS, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for eigenvalue vector\n");
		exit(0);
	}
	// 	cl_mem buffer_t  --  NoNodes vector  read-only  -- current tree
	// allocate upper bound MaxNodes
	InvInfo->buffer_t  = clCreateBuffer(context, CL_MEM_READ_ONLY, 
		sizeof(double)*MaxNodes, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for t (len) vector\n");
		exit(0);
	}
	// cl_mem buffer_i  -- NoNodes read only
	// allocate upper bound MaxNodes
	InvInfo->buffer_id  = clCreateBuffer(context, CL_MEM_READ_ONLY, 
	...
	InvInfo->buffer_temp  = clCreateBuffer(context, CL_MEM_READ_WRITE, 
		sizeof(double)*MaxNodes*NOS*NOS, NULL, &err);		
	if (err != 0) {
		printf("Error allocating OpenCL buffer for temporary matrix\n");
		exit(0);
	}	
...
void printSetPMatrixResults(RATES *Rates, TREES *Trees, OPTIONS *Opt, double RateMult, double Kappa) {
	...
	TREE* Tree;
	int NOS;
	// test: cal CPU version
	//btocl_SetAllPMatrixxx(Rates, Trees, Opt, RateMult, Kappa);
...
void setExpqtKernelInfo(TREES* Trees,TREE* Tree,cl_ushort kernel_type,KernelInfo* k) {
	...
	case BTOCL_EXPQT_NOTEMP:
		...
		break;
	case BTOCL_EXPQT3:
	case BTOCL_EXPQT3_LOCAL:
	case BTOCL_EXPQT3_LOCALERR:
		k->usewg = TRUE;
		k->check_error = 1;
		...
		break;
	case BTOCL_EXPQT3_NOS4: // uses private memory instead of local memory
		k->usewg = TRUE;
		k->check_error = 1;
		k->transpose_inv = 1;
		...
		//printf("NOS %d NoNOdes %d local %d global %d\n",NOS,NoNodes,k->localws[0],k->globalws[0]);
		break;
	case BTOCL_EXPQT_ROWNOS2:
	case BTOCL_EXPQT_ROWNOS4:
		k->usewg = TRUE;
		k->check_error = 1;
		k->transpose_inv = 1;
		wg_size = 512;  // multiple of 2 or 4
	...
	//	if (NoNodes % k->num_nodes_wg != 0)
	//		k->num_wg++;
	//	k->localws[0] = k->num_nodes_wg;        // row/column based
	//	k->globalws[0] = k->localws[0]*k->num_wg;
	//	k->localws[1] = NOS;
	//	k->globalws[1] = NOS;
		//printf("NOS %d NoNOdes %d local %d global %d\n",NOS,NoNodes,k->localws[0],k->globalws[0]);
	//	break;
	//case BTOCL_EXPQT_LOCAL:
	//	k->usewg = TRUE;
	//	k->num_nodes_wg = 2;
	//	k->num_wg = NoNodes / k->num_nodes_wg;
	//	if (NoNodes % k->num_nodes_wg != 0)
	//		k->num_wg++;
	//	k->localws[0] = NOS*NOS*k->num_nodes_wg;  // -- all cells
	//	k->globalws[0] = k->localws[0]*k->num_wg;
	//	break;
	// Below: we assume that a whole node can fit in a workgroup ie NOS*NOS < max_wg_size
	case BTOCL_EXPQT_LOCALERR:
		k->check_error = 1;
		// DONT add break, we want this to cascade down
	case BTOCL_EXPQT_LOCAL:
		k->usewg = TRUE;
...
// Sequential version
int	btocl_SetAllPMatrixxx(RATES *Rates, TREES *Trees, OPTIONS *Opt, double RateMult, double Kappa)
{
	double *TVect;
	int *IVect;
	TREE *Tree;
	...
	//{
	//	if (Index != IVect[Index]) printf("different %d %d\n",Index,IVect[Index]);
	//	printf("Index %d ID %d computedID %d\n",Index,Tree->NodeList[Index]->ID,IVect[Index]);
	//}
	//exit(0);
...
int	btocl_SetAllPMatrix(RATES *Rates, TREES *Trees, OPTIONS *Opt, double RateMult, double Kappa) {
	...
	switch(NOS) {
		case 2:
			...
			//kernel_type = BTOCL_EXPQT3_LOCAL;
	...
	standard_err = btocl_SetAllPMatrixxx(Rates, Trees, Opt, RateMult, Kappa);
...
int	btocl_SetAllPMatrixKernel(RATES *Rates, TREES *Trees, OPTIONS *Opt, double RateMult, double Kappa, cl_ushort kernel_type) {
	...
	*(Trees->perror) = 0;
	...
	if (err != FALSE) { printf("-->btocl version: error\n");  }
	...
	//printf("Finished computing...error checking: %d\n",scounter++);
...
void btocl_computePartialLhCPU(TREES *Trees, TREE* Tree, int group) {
	...
	{		
		...
		for(node_idx=0; node_idx < gsize; node_idx++)
		{
			...
			//printf("node id: \n",node->ID);
			for(site=0; site < Trees->NoOfSites; site++) {
...
void btocl_setPlhKernelInfo(TREES* Trees,TREE* Tree,int group,KernelInfo* k);
int btocl_setPlhArgs(cl_kernel kernel, TREES* Trees, TREE* Tree, KernelInfo k,  int group, int* err);
...
int btocl_computePartialLh(RATES* Rates, TREES *Trees, OPTIONS *Opt) {
	...
	//printGroupPlh(plh, Tree,0,NOS,NoOfSites);
	
	//memcpy(Trees->previous_plh,plh,sizeof(plh));
	
	//printGroupPlh(Trees->previous_plh, Tree,0,NOS,NoOfSites);
	//exit(0);
...
//	int lastGroup;
	...
	clEnqueueReadBuffer(queue,Trees->buffer_partialLh,CL_TRUE,0,
		sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS, plh,0,0,NULL);
		
		
		
	// debug
	//clEnqueueReadBuffer(queue,Trees->buffer_plhFactor,CL_TRUE,0,
	//	sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS, Trees->plhFactor,0,0,NULL);	
	// new clEnqueueReadBuffer(queue,Trees->buffer_plhFactor,CL_TRUE,0,
	//	sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS*Trees->max_nchildren, Trees->plhFactor,0,0,NULL);	
	...
	//	sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS, Trees->debug_plhFactor,0,0,NULL);	
	// new clEnqueueReadBuffer(queue,Trees->buffer_debug_plhFactor,CL_TRUE,0,
	//	sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS*Trees->max_nchildren, Trees->debug_plhFactor,0,0,NULL);	
...
void btocl_loadPartialLhBuffers(cl_command_queue queue, TREES* Trees, TREE *Tree, int NOS, int NoNodes) {
	...
	clEnqueueWriteBuffer(queue,Trees->buffer_partialLh,CL_TRUE,0,
		sizeof(double)*(Trees->NoOfSites)*NoNodes*NOS,
...
void btocl_setPlhKernelInfo(TREES* Trees,TREE* Tree,int group, KernelInfo* k) {
	...
	case BTOCL_PLH:
		...
		k->globalws[0] = (Trees->NoOfSites)*(group_size); // nodes per group
		if (k->globalws[0] < 20)
			k->passedCut = TRUE;
		break;
	case BTOCL_PLHNODE:
	case BTOCL_PLHNODE_NOS2:
		k->usewg = FALSE;	
	...
	case BTOCL_PLHROW:
		...
		if (group==0) // tips, compute only
			...
			k->action = 1;
		group_size = Tree->groupsIdx[2*group+1];
		k->globalws[0] = (Trees->NoOfSites)*(group_size)*NOS; // nodes per group	
		if (k->globalws[0] < 800) {
			k->passedCut = TRUE;
			k->action = 0; // collect
		}
		break;	
	case BTOCL_PLHROWG:  // one call per group, using workgroups
	case BTOCL_PLHROWGL:
		//printf("Here!!\n");
		if (group==0) // tips, compute only
			k->action = 2;
	...
	case BTOCL_PLHREDUCEGL:
		...
		break;				
	//case BTOCL_PLHROWALL:
	//	k->usewg = FALSE;	
	//	group_size = Tree->groupsIdx[1];  // size of Group 0
	//	k->globalws[0] = (Trees->NoOfSites)*(group_size)*NOS; // nodes per group	
	//	k->lastGroup = (Tree->NoFGroups)/2+1;
	//	break;	
	case BTOCL_PLHROWALLG:
		k->usewg = TRUE;
		// start from desired wg_size
...
//		break;
	default:
		printf("Kernel type not supported\n");
		exit(0);
	}	
}
...
void copyPlh(TREES* Trees,TREE* Tree,double* plh1, double* plh2) {
	...
	plh = plh2;
	plh_previous = plh1;
	for(i=0; i < Trees->NoOfSites * Trees->NoOfStates * Tree->NoNodes; i++) {
		*plh_previous = *plh;
		plh_previous++;
		plh++;
	}
