...
void	FindMLRagVals(TREES* Trees, TREE *Tree)
{
...
/*
double	FindMLVar(TREES* Trees, TREE *Tree, int Site1, double Alpha1, double Beta1, int Site2, double Alpha2, double Beta2, MODEL Model)
{
	double	Ret=0;
	int		x,y;
	CONVAR	*CV;
	...
	if(Model == CONTINUOUSREG)
	{
		for(x=0;x<Trees->NoOfTaxa;x++)
		{
			CV->TVect1[x] = Trees->Taxa[x].Dependant  - (Alpha1 + (Beta1 * Trees->Taxa[x].ConData[0]));
			CV->TVect3[x] = Trees->Taxa[x].Dependant  - (Alpha1 + (Beta1 * Trees->Taxa[x].ConData[0]));
			CV->TVect2[x] = 0;
		}
	}
	else
...
void	SetEstData(TREES *Trees, RATES* Rates)
{
	...
	for(TIndex=0;TIndex<Trees->NoOfTaxa;TIndex++)
	{
		if(Trees->Taxa[TIndex].EstData == TRUE)
			Trees->Taxa[TIndex].Dependant = Rates->EstData[ENo++];
	}
}
...
double	LHRandWalk(OPTIONS *Opt, TREES* Trees, RATES* Rates)
{
	...
	if((Rates->NoEstData > 0) || (Opt->UseVarData == TRUE))
	...
	for(Index=0;Index<Trees->NoOfTaxa;Index++)
		printf("%s\t", Trees->Taxa[Index].Name);	
	
	printf("\n");

	if(Tree->ConVars->TrueV != NULL)
		PrintMathematicaMatrix(Tree->ConVars->TrueV, "V = ", stdout);
	else
		PrintMathematicaMatrix(Tree->ConVars->V, "V = ", stdout);
